    ...
    uint gid [[thread_position_in_grid]])
{
    ...
    for (int ch = 0; ch < 3; ch++) {
        ...
        d_c[0 * 3 + ch] = SH_C0 * g;
        d_c[1 * 3 + ch] = -SH_C1 * y * g;
        d_c[2 * 3 + ch] = SH_C1 * z * g;
        d_c[3 * 3 + ch] = -SH_C1 * x * g;
        d_c[4 * 3 + ch] = SH_C2_0 * xy * g;
        d_c[5 * 3 + ch] = SH_C2_1 * yz * g;
        d_c[6 * 3 + ch] = SH_C2_2 * (2.0 * zz - xx - yy) * g;
        d_c[7 * 3 + ch] = SH_C2_3 * xz * g;
        d_c[8 * 3 + ch] = SH_C2_4 * (xx - yy) * g;

        float sh1 = c[1 * 3 + ch], sh2 = c[2 * 3 + ch], sh3 = c[3 * 3 + ch];
        float sh4 = c[4 * 3 + ch], sh5 = c[5 * 3 + ch], sh6 = c[6 * 3 + ch];
        float sh7 = c[7 * 3 + ch], sh8 = c[8 * 3 + ch];
